...
//            'misc' => 'required_if:from,0',
    ...
    public static function newInvoiceCreateValidation(Request $request)
    {
        return self::ValidatorMake($request->all(), [
            ...
            'invoice_items.*.product.required' => __('validation.required', ['attribute' => 'Product']),
            'invoice_items.*.product.exists' => __('validation.exists', ['attribute' => 'Product']),
            'invoice_items.*.quantity.required' => __('validation.required', ['attribute' => 'Quantity']),
            'invoice_items.*.quantity.numeric' => __('validation.numeric', ['attribute' => 'Quantity']),
            'invoice_items.*.price.required' => __('validation.required', ['attribute' => 'Price']),
            'invoice_items.*.price.numeric' => __('validation.numeric', ['attribute' => 'Price']),
            'invoice_items.*.tax.required' => __('validation.required', ['attribute' => 'Tax']),
            'invoice_items.*.tax.numeric' => __('validation.numeric', ['attribute' => 'Tax']),
            'invoice_items.array' => "At least one sales item is required",
            'invoice_items.required' => "At least one sales item is required",
            'invoice_items.min' => "At least one sales item is required",
        ]);
    }
