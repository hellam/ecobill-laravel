...
const KTGLGroupsUpdate = function () {
    ...
    const handleForm = function () {
        ...
        $(form.querySelector('[name="class_id"]')).on('change', function () {
            // Revalidate the field when an option is chosen
            validator.revalidateField("class_id");
        })
            ...
            if (validator) {
                validator.validate().then(function (status) {
                    ...
                    if (status === 'Valid') {
                        ...
                        const str = $('#kt_modal_update_gl_group_form').serialize()
                        handleSubmit(str)
                    } else {
                        Swal.fire({
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            if ($('#kt_gl_groups_table').length) {
                                $("#kt_gl_groups_table").DataTable().ajax.reload();
                                return;
                            }
                            // Redirect
